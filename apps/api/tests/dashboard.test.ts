...
test("dashboard endpoint delegates to the service scoped by merchantId", async () => {
  ...
  const service = {
    summary: async (merchantId: string) => {
      ...
      return { products: 1, orders: 2, customers: 3, conversations: 4, revenueMinor: 500, aiRevenueMinor: 200, averageOrderValueMinor: 250, ordersByStatus: { PAID: 1, CREATED: 1 }, currency: "INR" };
    },
